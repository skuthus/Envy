...
struct MarkupHelpView: View {
    ...
    private let groups: [Group] = [
        ...
        Group(title: "Links", entries: [
            ...
            Entry(syntax: "![[photo.png]]", description: "Embed an image. Drag a picture onto a note or paste one (a screenshot, a copied image) and this reference appears, rendering inline; add a size with ![[photo.png|400]] (width) or |400x300 (width×height). Right-click the image to resize, rename, open in Preview, or reveal it in Finder. The file is copied into the Attachments folder inside your Index; the original is left where it was. Search img: to find every note that has one."),
            Entry(syntax: "img:", description: "Show every note holding an image attachment. \"-img:\" excludes them; \"img: any words\" searches within them."),
            Entry(syntax: "title:word", description: "Match against titles only: the note *named* for a thing, not every note that mentions it. Several title: terms combine; quotes for phrases; \"-title:word\" excludes. Autocompletes note titles as you type."),
            Entry(syntax: "folder:name", description: "Notes filed under that folder, partial and case-insensitive like tag:, matched against the whole relative path, so folder:proj finds Projects and folder:work finds Projects/Work. Quoting demands exactness: folder:\"Work\" is that folder plus anything nested in it, never Workshop (clicking a folder dot produces this form). A bare \"folder:\" means any note in a subfolder; \"-folder:\" alone means notes at the Index root; \"-folder:name\" excludes one folder. Needs \"Show items in subfolders.\""),
            Entry(syntax: "ghost:", description: "Notes carrying an unresolved [[link]], one whose target note doesn't exist yet (those links render dimmed in the editor; clicking one creates the note). \"-ghost:\" finds notes whose links all resolve. Trailing words search within, like img:."),
            Entry(syntax: "interlink:Note Title", description: "Everything connected to that note, in either direction: the notes it links out to plus the notes linking back to it. The Interlinks footer as a searchable list. Quotes for spaced titles; \"-interlink:\" excludes the whole neighborhood; combines with anything (interlink:Project todo: finds open tasks across everything touching Project)."),
            Entry(syntax: "embed:", description: "Show every note that embeds another note. \"-embed:\" excludes them; \"embed: any words\" searches within them."),
        ]),
        Group(title: "Fleeting Notes", entries: [
            Entry(syntax: "inbox:", description: "Show the fleeting notes waiting in your Inbox: captures you haven't filed yet. They're ordinary notes living in an Inbox folder inside The Index, marked in the list with an amber !, and the count beside the search box says how many are waiting (click it to review, click again to come back)."),
        ...
        Group(title: "Tags", entries: [
            Entry(syntax: "#tag", description: "Tag: rendered bold with a tinted background. Search \"tag:name\" to find every note tagged that way, including partial matches; quote it (tag:\"name\") for an exact match, the form clicking a tag produces."),
        ]),
