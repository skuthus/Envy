...
/// The AI-agent guide for working with an Envy vault, in two forms:
///
/// - `guideBody` — tool-agnostic Markdown, written as `AGENTS.md` at the vault
///   root. Co-located with the notes, so *any* agent that works on the vault
///   finds it (AGENTS.md-aware tools read it automatically; others see it when
///   they list the folder).
/// - `claudeSkill` — the same body with Claude Code skill frontmatter, written
///   to ~/.claude/skills/envy-notes/SKILL.md for zero-config auto-surfacing in
///   Claude Code.
///
/// Kept in sync with the canonical copy in the repo at agent-skill/. The
/// closing delimiters sit at column 0 on purpose, so the Markdown is stored
/// byte-for-byte with no indentation stripped.
enum AgentSkillContent {

    /// Claude Code skill frontmatter (YAML). Harmless to other tools, but only
    /// prepended to the ~/.claude/skills copy; the vault AGENTS.md omits it.
    private static let claudeFrontmatter = """
---
name: envy-notes
description: >-
...
"""
    ...
    static var claudeSkill: String { claudeFrontmatter + guideBody }
