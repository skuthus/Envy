...
- **Trash, not gone for good** — Deleted notes go to a `Trash` folder in The Index (it syncs with your notes), restorable with `⌘⇧⌫` or browsable with `trash:`. Settings controls how often it gets swept into the real macOS Trash.
...
- **AI provenance** — AI provenance signatures in notes are left as plain text; the in-app badge is off pending a redesign.
