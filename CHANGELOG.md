...
- **Image embeds aren't note links.** A note that only has `![[photo.png]]` (and no `[[links]]` to other notes) now correctly shows up under `orphan:`. Searching `link:` only matches real note links, not attached pictures.
- **Tags in the `tag:` list are easier to read.** The bare `tag:` catalog no longer paints tags green, so the highlighted row stays clear. Colored tag chips in the title bar are unchanged.
...
- **Glassify.** A new toggle in Settings → Appearance turns Envy's chrome and content — the note list, editor, title bars, and footer — into frosted Liquid Glass floating over a deeper backdrop blur, so your desktop reads softly through the whole window. It's purely a look: every control, gesture, and layout stays exactly as it was.
...
- Deleting a note now sends it to a hidden `.trash` subfolder right next to it first, rather than straight to the real macOS Trash — restoring it (⌘⇧⌫) works the same as before, and always lands back in the exact folder it came from, since each folder gets its own `.trash`. Search `trash:` to browse everything currently trashed anywhere in The Index — clicking or arrowing through results just previews them read-only, with Restore/Reveal in Finder/Delete always a deliberate right-click or button away, never a side effect of browsing. Settings → General → Trash controls how often each `.trash` folder itself gets swept into the real macOS Trash ("Empty every _N_ Days/Weeks/Months," 30 days by default), and has a "Reveal Trash Folder in Finder" shortcut for the common top-level case.
- `template:` browsing no longer needs a separate "Edit Template" step — clicking or arrowing to a match opens it live, editable, and auto-saving right in the editor pane, the same click-to-open feel as a regular note. Creating a note from it is now its own deliberate action: press Return, click "Create Note from Template" in the editor pane's own header, or right-click for the same option.
- ⇧↑/⇧↓ now extend the note list's selection one at a time, same as Finder — growing or shrinking a multi-selection from the keyboard, not just ⇧-click. `template:` and `trash:` browsing now support the same ⇧-click/⌘-click/⇧↑/⇧↓ multi-select, with bulk Create Note from Template/Reveal in Finder/Move Back/Delete, or Restore/Reveal in Finder/Delete, in the right-click menu.
- Fixed wiki-link `[[` autocomplete not suggesting anything in the pinned-note popup or the option-click preview panel — both were missing the note-titles list the main editor already had.
- The note's title bar is no longer optional — removed the "Show title bar above note" toggle now that so much (rename, tags, the due-date pill) lives there.
- A long note title that's crowded by "Show tags in title bar" now scrolls into view on hover instead of just truncating, the same as the pinned-note popup's title already did.
...
- New: `![[Note Title]]`, on its own line followed by a blank line, embeds that note's live content inline instead of just linking to it — edit the source anywhere and every place it's embedded updates immediately. The link itself stays visible above the embed, exactly as typed; edit or delete it directly to change or remove the embed. Click the collapse arrow to shrink it down to just that link. Click into the embed to edit that note directly, and edits save straight back to it. Guards against embedding the note you're already looking at (would risk two competing saves on the same file), doesn't nest more than one level deep, and skips option-click preview on the link (it's already showing live right below). Scrolling through the host note past an embed's own box keeps scrolling the host — an embed only takes over the scroll wheel once the mouse has rested on it for a moment, not the instant the cursor crosses into it.
